const updateLog = (move) => {
    ...
    move.log.push([[piece], [move.from.notation], [move.to.notation], [move.captured]]);
};
...
const showLog = (move) => {
    ...
    for (let i = 0; i < move.log.length; i += 2) {
        let plus1 = move.log[i + 1] || '';
        log_table += `<tr><td class='white'>${move.log[i]}</td><td class='black'>${plus1}</td></tr>`
    };
    log_table += `</table>`;
